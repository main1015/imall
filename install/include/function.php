...
class installTestData
{
	...
	public static function onCreateController()
	{
		...
		$collectList = array
		(
			'http://list.jd.com/list.html?cat=1318,12099,9754',
			'http://list.jd.com/list.html?cat=1318,12099,9759',

			'http://list.jd.com/list.html?cat=1315,1342,1348',
			'http://list.jd.com/list.html?cat=1315,1342,9736',

			'http://list.jd.com/list.html?cat=1315,1343,9708',
			'http://list.jd.com/list.html?cat=1315,1343,11991',

			'http://list.jd.com/list.html?cat=9847,9848,9863',
			'http://list.jd.com/list.html?cat=9847,9848,9865',
		);
		$totalNum = count($collectList);
		foreach($collectList as $key => $val)
		{
			$percent = ($key+1/$totalNum);
			showProgress(array('isError' => false,'message' => '正在采集测试数据...','percent' => $percent));
			sleep(1);
			collect_facade::run('jd',$val,$num);
		}
